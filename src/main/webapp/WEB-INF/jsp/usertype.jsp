...
function formSubmit(){
	...
	var text=$("#text").val();
       ...
       function getDetails(status){
    	   ...
    	   ({
    	   type: "GET",
    	   url: "ajaxActions.htm?page=AM6&status="+status,
    	   dataType:"json",
    	   cache:false,
    	   async:false,
    	   success: function(data)
    	   {
			   ...
			   if(status == 'I' || status=='all'){
				   $("#bootstrap-table").empty();
	    		   var head_data="<thead><tr><th width='10%'>SNo</th><th width='20%'>User ID</th><th width='20%'>User Type</th><th width='20%'>Status</th></thead>";
	    		   var head_data1="<thead><tr><th width='10%'>SNo</th><th width='20%'>User ID</th><th width='20%'>User Type</th><th width='20%'>Status</th></thead>";
	    		   $(head_data).appendTo("#bootstrap-table");
			  ...
			  $("#bootstrap-table").empty();
    		   var head_data="<thead><tr><th width='10%'>SNo</th><th width='20%'>User ID</th><th width='20%'>User Type</th><th width='20%'>Status</th><th width='20%'>Action </th></thead>";
    		   var head_data1="<thead><tr><th width='10%'>SNo</th><th width='20%'>User ID</th><th width='20%'>User Type</th><th width='20%'>Status</th><th width='20%'>Action </th></thead>";
    		   $(head_data).appendTo("#bootstrap-table");
    		   $(head_data1).appendTo("#bootstrap-table");
    		   ...
    		   $("#bootstrap-table").empty();
    		   var head_data="<thead><tr><th width='10%'>SNo</th><th width='20%'>User ID</th><th width='20%'>User Type</th><th width='20%'>Status</th><th width='20%'>Action </th></thead>";
    		   var head_data1="<thead><tr><th width='10%'>SNo</th><th width='20%'>User ID</th><th width='20%'>User Type</th><th width='20%'>Status</th><th width='20%'>  </th></thead>";
    		   $(head_data).appendTo("#bootstrap-table");
    		   $(head_data1).appendTo("#bootstrap-table");
...
<div id="page-wrapper">
				...
				<div class="row">
					<ul class="nav navbar-top-links navbar-left">
						<li class="dropdown">
							...
							<a href="actions.htm?page=AM6&operation=0&status=A" class="left-breadcrumb-last-child">User Type</a>
							</h5>
						</li>
					</ul>
				</div>
	...
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header-title">User Type</h1>
		</div>
	</div>
	...
	<div class=" ">
		<div class="col-lg-12 col-md-12 search-config-align">
			...
			<br>
			<table class="table table-hover table-striped mst-table-align"
				id="bootstrap-table">
					...
					<form:form class="form-inline" name="form1" role="form">
						 ...
						 <div class="form-group hpclusers-popup">
								...
								<div class="devices-test">
									<label for="name"> User Type <b>*</b></label> <input
								type="text" name="description" class="form-control" id="text"
								placeholder="Enter UserType" onkeydown="clearData()">
									<div id="alert" class="alert-message"></div>
								</div>
								<div class="clear"></div>
								<input type="hidden" id="page" name="page" value="AM6"> <input
				...
				<div class="modal-body">
					...
					<div class="form-group  popup-form-align del-popup-button">
						...
						<form:form class="form-inline" action="actions.htm" method="post"
							...
							<input type="hidden" name="page" value="AM6">
							<input type="hidden" name="operation" value="3">
							<input type="hidden" id="message" name="userTypeID">
						...
						onsubmit="return upvalidate()">
						 ...
						 <div class="form-group hpclusers-popup">
							...
							<div id="upalert" class="alert-message"></div>
									...
									<div class="clear"> </div>
								<input type="hidden" name="page" value="AM6"> <input
								type="hidden" name="operation" value="2"> <input
								type="hidden" name="userTypeID" id="userTypeID">
								<button type="button" data-dismiss="modal"
								class="btn  btn-sm btn-primary popup-form-button-align" id="cancel" onclick="clearData()">Cancel</button>
							<button type="submit"
...
<script>
              ...
              {           	              
                if(formValidation(document.getElementById("text"),document.getElementById("alert"))){
                	 var text=$("#text").val();
             	    $.ajax({
            		     url:"validation.htm?page=AMUD4&description="+text,
            		   	 dataType:"json",
            		   	 cache:false,
            		   	 async:false,
            		     success: function (data) {
